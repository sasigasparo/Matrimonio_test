          ...
          <tbody>
            {dashboard.recent_logs.map(l => (
              <tr key={l.id} style={{ borderBottom:'1px solid var(--cream)' }}>
                <td data-label="Date" style={{ padding:'10px 16px', color:'var(--warm-gray)', whiteSpace:'nowrap' }}>{new Date(l.created_at).toLocaleString('en-GB')}</td>
                <td data-label="User" style={{ padding:'10px 16px', color:'var(--charcoal)', fontWeight:500 }}>{l.actor}</td>
                <td data-label="Action" style={{ padding:'10px 16px', color:'var(--charcoal)' }}>{LOG_LABELS[l.action] || l.action}</td>
