    ...
    >
      <div style={{ display: 'flex', gap: 14, alignItems: 'center', padding: '14px 16px' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: 600, color: 'var(--charcoal)', fontSize: '.95rem' }}>{meal.strMeal}</div>
          <div style={{ fontSize: '.78rem', color: 'var(--warm-gray)', marginTop: 2 }}>
      ...
      <div style={{
        ...
        <p style={{
          ...
          fontSize: '.85rem', color: 'var(--warm-gray)', lineHeight: 1.7,
          borderTop: '1px solid var(--cream)', paddingTop: 12,
        }}>
          {meal.strInstructions?.slice(0, 320)}…
        </p>
      </div>
    </div>
  )
...
function MealDBWidget() {
  const [meals, setMeals]     = useState([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    Promise.all(
      PIATTI_NAPOLETANI.map(name =>
